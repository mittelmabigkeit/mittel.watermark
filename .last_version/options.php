...
CModule::IncludeModule('mittel.watermark');
...
if( (isset($_REQUEST["save"]) || isset($_REQUEST["apply"])) && check_bitrix_sessid()){
	__AdmSettingsSaveOptions('mittel.watermark',$arAllOptions['wm_settings_file']);
	COption::SetOptionString('mittel.watermark', 'watermark_file', $_REQUEST['watermark_file']);
	__AdmSettingsSaveOptions('mittel.watermark',$arAllOptions['wm_settings_text']);
	COption::SetOptionString('mittel.watermark', 'watermark_text_font', $_REQUEST['watermark_text_font']);
	__AdmSettingsSaveOptions('mittel.watermark',$arAllOptions['data']);
	foreach($arrIBlocks as $arFields){
		if(!in_array($arFields['ID'],$arIBlocksOpt))
			continue;
		__AdmSettingsSaveOptions('mittel.watermark',$arAllOptions['iblock_'.$arFields['ID']]);
	}
...
?><form method="post" action="<?=$APPLICATION->GetCurPage()?>?mid=<?=urlencode($mid)?>&amp;lang=<?=LANGUAGE_ID?>" name="imyie_wm"><?
	...
	/*__AdmSettingsDrawRow('mittel.watermark', GetMessage('IMYIE.MORE_WATERMARK.WM_MAIN_SETTINGS'));
	__AdmSettingsDrawList('mittel.watermark', $arAllOptions['wm_main_settings']);*/
	__AdmSettingsDrawRow('mittel.watermark', GetMessage('IMYIE.MORE_WATERMARK.WM_SETTINGS'));
	__AdmSettingsDrawList('mittel.watermark', $arAllOptions['wm_settings_file']);
	?><tr><?
	...
	__AdmSettingsDrawList('mittel.watermark', $arAllOptions['wm_settings_text']);
	?><tr><?
	...
	__AdmSettingsDrawRow('mittel.watermark', GetMessage('IMYIE.MORE_WATERMARK.DATA'));
	__AdmSettingsDrawList('mittel.watermark', $arAllOptions['data']);
	...
	if (is_array($arIBlocksOpt) && count($arIBlocksOpt)>0 && $arIBlocksOpt[0]!=0){
			...
			?><td colspan="2"><?
				...
				foreach($arrIBlocks as $arFields){
					...
					?><table class="internal" style="min-width:250px;float:left;margin-right:20px;margin-bottom:20px;"><?
						?><tbody><?
							__AdmSettingsDrawRow('mittel.watermark', $arFields['NAME']);
							__AdmSettingsDrawList('mittel.watermark', $arAllOptions['iblock_'.$arFields['ID']]);
						?></tbody><?
					?></table><?
				}
		...
		__AdmSettingsDrawRow('mittel.watermark', array('note'=>GetMessage('IMYIE.MORE_WATERMARK.EMPTY_IBLOCKS')));
	}
